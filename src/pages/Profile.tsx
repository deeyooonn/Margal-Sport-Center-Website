...
export function Profile() {
  ...
  // Load user profile details from localStorage or use brand defaults
  const [profile, setProfile] = useState(() => {
    try {
      ...
      console.error('Error reading margal_user_profile from localStorage', e);
    }
    return {
      name: 'Juan Dela Cruz',
  ...
  // Official Sports Avatar Choices from Unsplash
  const sportsAvatars = [
    {
      name: 'Elite Player',
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Dispatch custom event for real-time header profile widget updates!
  ...
  const handleCancel = () => {
    ...
    setEditFB(profile.fbLink);
    setSelectedAvatar(profile.avatar);
    setEditRole(profile.role || 'user');
    setIsEditing(false);
  };

  // Sync with localStorage bookings
    ...
    try {
      ...
      if (stored) {
        ...
        return parsed.map((b: any) => ({
          id: b.id,
          dateString: new Date(b.date).toLocaleDateString('en-US', {
            weekday: 'long',
            month: 'long',
            day: 'numeric',
            year: 'numeric'
          }),
          slots: b.slots.map((s: any) => s.timeLabel),
          total: b.total,
  ...
  // Prepopulated premium historical bookings (to show completed games)
  const pastBookings: PastBooking[] = useMemo(() => {
    const activeConfirmed = confirmedBookings;
    const history: PastBooking[] = [
      {
        id: 'past-1',
    ...
    return [...activeConfirmed, ...history];
  }, [confirmedBookings]);
    ...
    <div className="flex-grow bg-slate-50 dark:bg-slate-950 py-12 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="text-center sm:text-left space-y-2">
              <div className="flex flex-wrap justify-center sm:justify-start items-center gap-2.5">
                <h1 className="text-3xl font-display font-bold uppercase tracking-wide text-slate-900 dark:text-white">
                  {isEditing ? editName || 'Juan Dela Cruz' : profile.name}
                </h1>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm font-semibold flex items-center justify-center sm:justify-start gap-1.5">
                ...
                <span>Member since {profile.createdDate}</span>
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Left Column: Account Details (Static View or Interactive Form) */}
                ...
                <form onSubmit={handleSave} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="facebook.com/ka.margal.player"
                        className="w-full pl-10 pr-3 py-2 border border-slate-300 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-pastel-blue focus:border-transparent text-sm font-semibold"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      >
                        <option value="user" className="bg-white text-slate-900 dark:bg-slate-800 dark:text-white">User (Standard Account)</option>
                        <option value="admin" className="bg-white text-slate-900 dark:bg-slate-800 dark:text-white">Admin (Override Account)</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex gap-3">
                    ...
                    <button
                      type="submit"
                      className="w-full py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 font-semibold transition-all flex items-center justify-center gap-1.5 cursor-pointer text-xs shadow-sm"
                    >
                      <Save size={14} /> Save
                    </button>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-100/50 dark:border-slate-800">
                    ...
                    <div>
                      ...
                      <a
                        href={profile.fbLink.startsWith('http') ? profile.fbLink : `https://${profile.fbLink}`}
                        target="_blank"
                  ...
                  <div className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-100/50 dark:border-slate-800">
                    ...
                    <div>
                      ...
                      <div className={`text-sm font-bold mt-0.5 ${profile.role === 'admin' ? 'text-pastel-blue' : 'text-slate-700 dark:text-slate-300'}`}>
                        {profile.role === 'admin' ? '🔑 Admin Override Mode' : '👤 Standard Customer'}
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {pastBookings.length === 0 ? (
                  <div className="text-center py-10 text-slate-400 text-sm font-semibold">
                    No confirmed bookings yet. Go to <Link to="/book" className="text-pastel-blue hover:underline">Book Court</Link> to reserve your slot!
                  </div>
                ) : (
                  pastBookings.map((b) => (
                    <div
                      key={b.id}
                      className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-3 relative overflow-hidden group hover:border-pastel-blue/40 transition-colors"
                    >
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`text-[9px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-full ${
                            b.status === 'completed'
                              ? 'bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300'
                              : 'bg-pastel-coral/10 text-pastel-coral dark:text-pastel-coral-light'
                          }`}
                        >
                          {b.status}
                        </span>
